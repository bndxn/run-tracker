...
log_path = os.path.join("/tmp", "garmindb.log")
...
def __get_date_and_days(db, latest, table, col, stat_name):
    if latest:
        ...
        if last_ts is None:
            ...
            logger.info(
                "Recent %s data not found, using: %s : %s", stat_name, date, days
            )
        else:
            # start from the day before the last day in the DB
            logger.info("Downloading latest %s data from: %s", stat_name, last_ts)
            last_ts_date_date = (
                last_ts.date() if isinstance(last_ts, datetime.datetime) else last_ts
            )
    ...
    if date is None or days is None:
        logger.error(
            "Missing config: need %s_start_date and download_days. Edit GarminConnectConfig.py.",
            stat_name,
        )
        sys.exit()
    return (date, days)
...
def import_data(debug, latest, stats):
    ...
    if Statistics.activities in stats:
        ...
        gjsd = GarminJsonSummaryData(
            db_params_dict, activities_dir, latest, measurement_system, debug
        )
        if gjsd.file_count() > 0:
            gjsd.process()

        gdjd = GarminJsonDetailsData(
            db_params_dict, activities_dir, latest, measurement_system, debug
        )
...
def basecamp_activity(debug, export_activity_id):
    ...
    logger.info(
        "Opening activity %d (%s) in BaseCamp", export_activity_id, file_with_path
    )
...
def google_earth_activity(debug, export_activity_id):
    ...
    file_with_path = export_activity(debug, tempfile.mkdtemp(), export_activity_id)
    logger.info(
        "Opening activity %d (%s) in GoogleEarth", export_activity_id, file_with_path
    )
...
def main(argv):
    ...
    parser.add_argument(
        "-t", "--trace", help="Turn on debug tracing", type=int, default=0
    )
    modes_group = parser.add_argument_group("Modes")
    modes_group.add_argument(
        "-b",
        "--backup",
        help="Backup the database files.",
        dest="backup_dbs",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "-d",
        "--download",
        help="Download data from Garmin Connect for the chosen stats.",
        dest="download_data",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "-c",
        "--copy",
        help="copy data from a connected device",
        dest="copy_data",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "-i",
        "--import",
        help="Import data for the chosen stats",
        dest="import_data",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "--analyze",
        help="Analyze data in the db and create summary and derived tables.",
        dest="analyze_data",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "--rebuild_db",
        help="Delete Garmin DB db files and rebuild the database.",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "--delete_db",
        help="Delete Garmin DB db files for the selected activities.",
        action="store_true",
        default=False,
    )
    modes_group.add_argument(
        "-e",
        "--export-activity",
        help="Export an activity to a TCX file based on the activity's id",
        type=int,
    )
    modes_group.add_argument(
        "--basecamp-activity", help="Export an activity to Garmin BaseCamp", type=int
    )
    modes_group.add_argument(
        "-g",
        "--google-earth-activity",
        help="Export an activity to Google Earth",
        type=int,
    )
    ...
    stats_group.add_argument(
        "-A",
        "--all",
        help="Download and/or import data for all enabled stats.",
        action="store_const",
        dest="stats",
    ...
    stats_group.add_argument(
        "-a",
        "--activities",
        help="Download and/or import activities data.",
        dest="stats",
        action="append_const",
    ...
    stats_group.add_argument(
        "-m",
        "--monitoring",
        help="Download and/or import monitoring data.",
        dest="stats",
        action="append_const",
    ...
    stats_group.add_argument(
        "-r",
        "--rhr",
        help="Download and/or import resting heart rate data.",
        dest="stats",
        action="append_const",
    ...
    stats_group.add_argument(
        "-s",
        "--sleep",
        help="Download and/or import sleep data.",
        dest="stats",
        action="append_const",
    ...
    stats_group.add_argument(
        "-w",
        "--weight",
        help="Download and/or import weight data.",
        dest="stats",
        action="append_const",
    ...
    modifiers_group.add_argument(
        "-l",
        "--latest",
        help="Only download and/or import the latest data.",
        action="store_true",
        default=False,
    )
    modifiers_group.add_argument(
    ...
    if args.rebuild_db:
        delete_dbs(
            [stats_to_db_map[stat] for stat in gc_config.enabled_stats()] + summary_dbs
        )
